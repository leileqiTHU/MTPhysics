...
def parse_args():
    ...
    parser.add_argument(
        "--shuffle_and_repack_before_every_epoch",
        type=bool,
        default=False,
        help="Whether to shuffle and repack the dataset before every epoch's training",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        "--train_file", type=str, default='/data1/leileqi/MTPhysics/deep_alignment/llama3-ultrafeedback-hybrid-v2_train_pretrain_format.jsonl', help="A csv, txt or a json file containing the training data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tokenizer_name",
        type=str,
        default='/data1/public_ckpts/gpt2/', # use gpt2's trained tokenizer
    ...
    parser.add_argument("--output_dir", type=str, default='/data1/leileqi/MTPhysics/pretrain/pretrained_models_Ultra_MTP_from_scratch_No-ShuffleDataEveryEpoch ', help="Where to store the final model.")
...
def main():
    ...
    send_example_telemetry("run_clm_no_trainer_Ultra_continue_pretrain", args)
    ...
    from modeling_mtp import GPTNeoXForCausalLM_MTP

    if args.model_name_or_path:
        model = GPTNeoXForCausalLM_MTP.from_pretrained(
        ...
        model = GPTNeoXForCausalLM_MTP(config)
    ...
    def tokenize_function(examples):
        ...
        new_bio = []
        for b in bio:
            b = b+tokenizer.eos_token
            new_bio.append(b)
        return tokenizer(new_bio)
    ...
    def group_texts(examples):
        ...
        total_length = len(concatenated_examples[list(examples.keys())[0]]) #input_ids
    ...
    with accelerator.main_process_first():
        ...
        lm_datasets = tokenized_datasets.shuffle(seed=seed).map( # every process concantenate the datasets in different orders
            group_texts,
            batched=True,
            num_proc=args.preprocessing_num_workers,
    ...
    optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=args.learning_rate, weight_decay=0.1, eps=1e-6)
    ...
    model, optimizer, eval_dataloader, lr_scheduler, train_dataloader = accelerator.prepare(
        model, optimizer, eval_dataloader, lr_scheduler, train_dataloader
    )
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("clm_no_trainer_UltraPretrain", experiment_config)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        # # shuflle the dataset and concat into a new train_dataloader
        if args.shuffle_and_repack_before_every_epoch:
            with accelerator.main_process_first():
        ...
        # In the main training loop (inside the epoch loop):
        total_correct = 0
        total_tokens = 0
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                shifted_labels = batch["labels"][:, 1:]  # Remove first token
                
                preds = shifted_logits.argmax(dim=-1)
                correct = (preds == shifted_labels).sum()
                total = torch.tensor(shifted_labels.numel(), device=accelerator.device)
            ...
            accelerator.log(
                {
                    ...
                    "validation_NTP_ACC": eval_accuracy,
                },
                step=completed_steps,
            )

        if epoch < args.num_train_epochs - 1 and epoch%1==0:
